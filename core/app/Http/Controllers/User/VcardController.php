...
class VcardController extends Controller
{
    ...
    public function projectStore(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $filename = time() . '.' . $img->getClientOriginalExtension();
            $directory = 'assets/front/img/user/projects/';
            if (!file_exists($directory)) mkdir($directory, 0775, true);
            $request->file('image')->move($directory, $filename);
            $input['image'] = $filename;
        }
    ...
    public function projectUpdate(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $filename = time() . '.' . $img->getClientOriginalExtension();
            $request->file('image')->move('assets/front/img/user/projects/', $filename);
            if (file_exists('assets/front/img/user/projects/' . $project->image)) {
    ...
    public function testimonialUpdate(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $filename = time() . '.' . $img->getClientOriginalExtension();
            $request->file('image')->move('assets/front/img/user/testimonials/', $filename);
            if (file_exists('assets/front/img/user/testimonials/' . $testimonial->image)) {
