<!--====== Footer Part Start ======-->
@if((isset($home_sections->top_footer_section) && $home_sections->top_footer_section == 1) || (isset($home_sections->copyright_section) && $home_sections->copyright_section == 1))
<footer>
    <div class="container">
                ...
                <div class="row">
                        ...
                        <div class="widget site-info-widget">
                            @if(isset($userFooterData->logo))
                                <a class="footer-logo" href="{{route('front.user.detail.view', getParam())}}">
                                    <img class="lazy" data-src="{{asset('assets/front/img/user/footer/'. $userFooterData->logo)}}" alt="Footer Logo">
                                </a>
                            @endif
                    ...
                    <div class="col-lg-8 col-sm-7 order-2">
                        <div class="widget newsletter-widget">
                            <h4 class="widget-title">{{$keywords['SUBSCRIBE_FOR_NEWSLETTER'] ?? 'SUBSCRIBE FOR NEWSLETTER'}}</h4>
                            <div class="newsletter-form">
                                <form action="{{route('front.user.subscriber',getParam())}}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <input type="email" placeholder="{{$keywords["Email_Address"] ?? "Email Address"}}" name="email" required>
                                    <button type="submit" class="main-btn">{{$keywords['Subscribe'] ?? 'Subscribe'}}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                                ...
                                @foreach($userFooterRecentBlogs as $footerRecentBlog)
                                    <div class="single-post dis-flex mb-4">
                                        <div class="post-thumb">
                                            <img class="lazy" data-src="{{ asset('assets/front/img/user/blogs/' . $footerRecentBlog->image) }}" alt="Image" height="60" width="80">
                                        </div>
                                        <div class="post-desc post">
                                            <span class="date"><i class="far fa-calendar-alt mar-right-4"></i>{{\Carbon\Carbon::parse($footerRecentBlog->created_at)->format('F j, Y')}}</span>
                                            
                                            <a href="{{route('front.user.blog.detail', [getParam(), $footerRecentBlog->slug, $footerRecentBlog->id])}}">
    ...
    @if(isset($home_sections->top_footer_section) && $home_sections->top_footer_section == 1)
    <!-- Lines -->
    <img data-src="{{asset('assets/front/user/img/lines/01.png')}}" alt="line-shape" class="line-one lazy">
    <img data-src="{{asset('assets/front/user/img/lines/02.png')}}" alt="line-shape" class="line-two lazy">
    @endif
</footer>
@endif
<!--====== Footer Part end ======-->
